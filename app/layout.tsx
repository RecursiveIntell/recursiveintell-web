import type { Metadata } from "next";
import { DM_Serif_Display, Instrument_Serif, JetBrains_Mono, Space_Grotesk } from "next/font/google";
import { SiteFooter } from "@/components/SiteFooter";
import { SiteHeader } from "@/components/SiteHeader";
import { ChatProvider } from "@/components/chat/ChatProvider";
import { ChatWidget } from "@/components/chat/ChatWidget";
import { ThemeProvider } from "@/components/ThemeProvider";
import { CommandPalette } from "@/components/CommandPalette";
import "./globals.css";

const display = DM_Serif_Display({
  variable: "--font-display",
  subsets: ["latin"],
  weight: "400",
});

const italicSerif = Instrument_Serif({
  variable: "--font-serif-it",
  subsets: ["latin"],
  weight: "400",
  style: ["normal", "italic"],
});

const sans = Space_Grotesk({
  variable: "--font-sans",
  subsets: ["latin"],
});

const mono = JetBrains_Mono({
  variable: "--font-mono",
  subsets: ["latin"],
});

const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || "https://recursiveintell.com";

export const metadata: Metadata = {
  metadataBase: new URL(siteUrl),
  title: {
    default: "recursiveintell · Josh Stevenson",
    template: "%s · recursiveintell",
  },
  description:
    "Josh Stevenson builds a local flight recorder for agent work: semantic memory, claim/evidence ledgers, TurboQuant compression research, Gloss, and ESP32-S3 edge AI.",
  keywords: [
    "Josh Stevenson",
    "Rust engineer",
    "portfolio",
    "Rust",
    "agent memory",
    "semantic memory",
    "MCP",
    "claim ledger",
    "context governor",
    "TurboQuant",
    "PolyKV",
    "Gloss",
    "ESP32-S3",
    "local-first",
    "evidence runtime",
    "crates.io",
    "RecursiveIntell",
  ],
  authors: [{ name: "Josh Stevenson", url: siteUrl }],
  creator: "Josh Stevenson",
  openGraph: {
    type: "website",
    locale: "en_US",
    url: siteUrl,
    siteName: "RecursiveIntell",
    title: "recursiveintell · Josh Stevenson",
    description:
      "Local flight recorder for agents: memory, claims, receipts, TurboQuant research, Gloss, and ESP32 edge AI.",
    images: [
      {
        url: "/images/ai_pic.png",
        width: 1200,
        height: 630,
        alt: "Josh Stevenson - Rust Evidence Runtime Engineer",
      },
    ],
  },
  twitter: {
    card: "summary_large_image",
    title: "recursiveintell · Josh Stevenson",
    description:
      "Agent evidence stack, published compression crates, Gloss, and measured ESP32 edge AI.",
    images: ["/images/ai_pic.png"],
  },
  robots: {
    index: true,
    follow: true,
    googleBot: {
      index: true,
      follow: true,
      "max-video-preview": -1,
      "max-image-preview": "large",
      "max-snippet": -1,
    },
  },
  alternates: {
    canonical: siteUrl,
  },
};

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  const personJsonLd = {
    "@context": "https://schema.org",
    "@type": "Person",
    "@id": `${siteUrl}/#person`,
    name: "Josh Stevenson",
    alternateName: ["JSense", "RecursiveIntell"],
    url: siteUrl,
    image: `${siteUrl}/images/ai_pic.png`,
    email: "josh@recursiveintell.com",
    jobTitle: "Rust Evidence Runtime Engineer",
    description:
      "Rust-focused systems engineer building local-first memory, evidence, verification, and operator-control runtimes.",
    knowsAbout: [
      "Rust",
      "Tauri",
      "React",
      "SQLite",
      "Semantic Memory",
      "Evidence Runtime",
      "Verification Systems",
      "ClaimLedger",
      "Local-First Software",
      "Operator Control Systems",
      "Linux",
    ],
    hasCredential: [
      {
        "@type": "EducationalOccupationalCredential",
        name: "Magna Cum Laude",
        description: "Graduated with high academic distinction",
      },
    ],
    memberOf: [
      {
        "@type": "Organization",
        name: "Phi Theta Kappa",
        description: "International honor society for academic excellence",
      },
      {
        "@type": "Organization",
        name: "Sigma Kappa Delta",
        description: "National English honor society",
      },
    ],
    sameAs: [
      "https://github.com/RecursiveIntell",
      "https://x.com/RecursiveIntell",
    ],
  };

  const websiteJsonLd = {
    "@context": "https://schema.org",
    "@type": "WebSite",
    "@id": `${siteUrl}/#website`,
    url: siteUrl,
    name: "RecursiveIntell",
    description: "Rust evidence-runtime portfolio and lab workbench",
    author: { "@id": `${siteUrl}/#person` },
    potentialAction: {
      "@type": "SearchAction",
      target: {
        "@type": "EntryPoint",
        urlTemplate: `${siteUrl}/?search={search_term_string}`,
      },
      "query-input": "required name=search_term_string",
    },
  };

  const jsonLd = [personJsonLd, websiteJsonLd];

  return (
    <html lang="en" suppressHydrationWarning>
      <head>
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }}
        />
      </head>
      <body
        className={`${display.variable} ${italicSerif.variable} ${sans.variable} ${mono.variable} antialiased`}
      >
        <ThemeProvider>
          <ChatProvider>
            <a
              href="#main-content"
              className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-50 focus:rounded-lg focus:bg-[color:var(--color-accent)] focus:px-4 focus:py-2 focus:text-white focus:outline-none"
            >
              Skip to main content
            </a>
            <div className="min-h-screen flex flex-col">
              <SiteHeader />
              <main id="main-content" className="flex-1 pb-16">
                {children}
              </main>
              <SiteFooter />
            </div>
            <ChatWidget />
            <CommandPalette />
          </ChatProvider>
        </ThemeProvider>
      </body>
    </html>
  );
}
